...
interface VivaBotProps {
  onNavigate?: () => void;
}

const VivaBot = ({ onNavigate }: VivaBotProps) => {
  ...
  // Agent switching service - 5 questions per agent, then switch
  const agentSwitchingServiceRef = useRef<AgentSwitchingService | null>(null);
  ...
  // Initialize with academic avatar automatically
  useEffect(() => {
    if (!selectedAvatar) {
    ...
    onMessage: (message) => {
      ...
      if (role === "agent") {
        ...
        if (currentAvatar && text) {
          ...
          if (agentSwitchingServiceRef.current) {
            agentSwitchingServiceRef.current.recordQuestion();
          }
        }
      } else if (role === "user") {
        ...
        if (text) {
          ...
          // After user answers, check if we need to switch agents
          if (agentSwitchingServiceRef.current) {
            const nextAgent = agentSwitchingServiceRef.current.recordAnswer();
            if (nextAgent) {
              console.log("🔄 Auto-switching to next agent after", agentSwitchingServiceRef.current.getCurrentQuestionCount(), "answers");
              // Wait a bit before switching to let the current answer be processed
              setTimeout(() => {
                handleSwitchAvatar(nextAgent);
              }, 2000);
            }
          }
        }
  ...
  const handleStartInterview = async () => {
    ...
    // Initialize agent switching service with agent order
    const agentOrder = Object.keys(avatarDetails);
    agentSwitchingServiceRef.current = new AgentSwitchingService({
      questionsPerAgent: 6,
      agentOrder: agentOrder
    });
    console.log("🎯 Agent switching initialized: 6 questions per agent, order:", agentOrder);
  ...
  const handleEndInterview = async () => {
    try {
      ...
      setMessages([]);
      setSpeakingAvatar(null);
      // Reset agent switching service
      if (agentSwitchingServiceRef.current) {
        agentSwitchingServiceRef.current.reset();
      }
    } catch (err) {
  ...
  const handleSendTextMessage = () => {
    ...
    // Track user answer and check if we need to switch agents
    if (agentSwitchingServiceRef.current) {
      const nextAgent = agentSwitchingServiceRef.current.recordAnswer();
      if (nextAgent) {
        console.log("🔄 Auto-switching to next agent after", agentSwitchingServiceRef.current.getCurrentQuestionCount(), "answers");
        ...
        const currentCount = agentSwitchingServiceRef.current.getCurrentQuestionCount();
        console.log(`📊 Answer ${currentCount}/6 recorded for current agent`);
      }
    }
        ...
        <div className="pt-8 md:pt-16 pb-4 md:pb-8 px-4">
          {onNavigate && (
            <div className="flex justify-end mb-4">
              ...
              >
                <span>Switch to Interview Bot</span>
          ...
          <div className="text-center">
            <h1 className="text-3xl md:text-5xl font-bold text-gray-900 mb-2 md:mb-4">
              Welcome to Your Internship Viva Bot
            </h1>
            <p className="text-base md:text-xl text-gray-600">
              Submit your internship report and start the viva
            </p>
            ...
            <div className="flex gap-2 md:gap-3 items-end">
              <div className="flex-1 bg-white border-2 border-gray-300 rounded-2xl px-4 md:px-6 py-3 md:py-4 flex items-center gap-2 md:gap-3 shadow-lg focus-within:border-[#A84448] transition-colors">
                <input
                  ...
                  placeholder="Describe your internship topic (optional)..."
    ...
    <div className="h-screen flex flex-col bg-gradient-to-br from-red-50 via-white to-rose-50">
      {onNavigate && (
        <div className="absolute top-4 right-4 z-50">
          <button
            onClick={onNavigate}
            className="px-3 py-1.5 bg-gradient-to-r from-blue-500 to-blue-600 text-white text-sm rounded-lg hover:shadow-lg transition-all flex items-center gap-2"
          >
